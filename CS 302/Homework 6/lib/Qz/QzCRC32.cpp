...
//	File: CRC32.cpp
//
//	$Header: /Projects/Qz/QzCRC32.cpp  8  2010/3/19 5:00:02p  Lee $
//
//
...
static const U32 g_CrcTable[256] =
{
...
/*
// The following chunk of code will generate the above table.
U32 table[256];
...
//	constructor
//
//	Instantiate a new instance of a CRC32 object, setting the CRC value
//	in preparation for immediate use.
//
QzCRC32::QzCRC32(void)
{
	Reset();
}
...
//
U32 QzCRC32::RetrieveCRC(void)
{
	// Return the bitwise complement of the CRC value.
	return ~m_CRC;
}
...
//	generated.
//
void QzCRC32::AccumulateBuffer(const U08 buffer[], U32 bufferSize)
{
	if (NULL == buffer) {
		return;
	}
	...
	for (U32 index = 0; index < bufferSize; ++index) {
		accum = ((accum >> 8) & 0xFFFFFF) ^ g_CrcTable[(accum ^ buffer[index]) & 0xFF];
	}
...
U32 QzComputeCrc32(const U08 *pSrc, U32 byteCount)
{
	...
	U32 accum = 0xFFFFFFFF;
	for (U32 index = 0; index < byteCount; ++index) {
		accum = ((accum >> 8) & 0xFFFFFF) ^ g_CrcTable[(accum ^ pSrc[index]) & 0xFF];
	}
...
//	QzComputeAdlerCrc32()
//
//	Alternate CRC routine used in PNGs for running error testing while
//	deflating the pixel data.
//
...
U32 VssCrc32(const U08 buffer[], U32 bufferSize)
{
	...
	// Do not apply the bitwise complement.
	return accum;
}
...
//
//	Alternate version of the CRC-32 logic used for VSS.  It starts from zero
//	instead -1, and XORs the two 16-bit words together to produce the final
//	CRC value.
//
